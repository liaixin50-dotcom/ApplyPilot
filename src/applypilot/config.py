...
def get_tier() -> int:
    ...
    has_llm = (
        ...
        or shutil.which("claude") is not None  # Claude CLI = zero-config Tier 2
    )
    if not has_llm:
        return 1
...
def check_tier(required: int, feature: str) -> None:
    ...
    missing: list[str] = []
    if required >= 2:
        has_api = any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL"))
        has_claude_cli = shutil.which("claude") is not None
        if not has_api and not has_claude_cli:
            missing.append("LLM API key — run [bold]applypilot init[/bold] or set GEMINI_API_KEY")
            missing.append("Alternatively, install [bold]Claude Code CLI[/bold] (claude.ai/code) for zero-config AI scoring")
    if required >= 3:
